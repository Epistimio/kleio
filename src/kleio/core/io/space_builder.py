...
"""
:mod:`kleio.core.io.space_builder` -- Create Space objects from configuration
=============================================================================
...
arguments with kleio's keywords for declaring hyperparameter types
to be optimized.

Motivation for this way of kleio's configuration is to achieve as
minimal intrusion to user's workflow as possible by:
   ...
   * Instead of passing the actual hyperparameter values, use one of
     ...
     or `kleio.core.io.space_builder.DimensionBuilder`,
...
from kleio.core.io.convert import infer_converter_from_file_type
...
class SpaceBuilder(object):
    ...
    # TODO Expose these 4 USER oriented goodfellows to a kleio configuration file :)
    USERCONFIG_KEYWORD = 'kleio~'
    USERARGS_TMPL = r'(.*)~([\+\-\>]?.*)'
    ...
    def build_to(self, config_path, trial, experiment=None):
        ...
        :param config_path: Path in which the configuration file instance
           will be created.
        :param trial: A `kleio.core.worker.trial.Trial` object with concrete
           parameter values for the defined `Space`. It may be used to retrieve
           managerial information from it.
        :param experiment: An `kleio.core.worker.experiment.Experiment` object.
